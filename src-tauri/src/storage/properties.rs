...
fn parse_line(line: &str) -> Option<Property> {
    ...
    // Values are stored and handed back verbatim (still in server.properties
    // escaped form). The MOTD is the only field that needs those escapes
    // decoded for display, and the frontend owns that (see motd.ts) — decoding
    // here as well would double-escape it on the round-trip back to disk.
...
mod tests {
    ...
    #[test]
    fn values_are_stored_and_written_verbatim() {
        // The MOTD reaches us already in server.properties escaped form — the
        // frontend encodes the section sign as the six characters \ u 0 0 A 7.
        // We must round-trip that byte-for-byte; re-escaping the backslash here
        // would produce \\u00A7 and the game would show the literal escape.
        let encoded_motd = "\\u00A7lHello\\u00A7oWorld";
        let properties = parse(&format!("motd={encoded_motd}\n"));
        assert_eq!(properties[0].value, encoded_motd);

        let merged = merge("motd=old\n", &properties);
        assert!(merged.contains(&format!("motd={encoded_motd}")));
    }
}
